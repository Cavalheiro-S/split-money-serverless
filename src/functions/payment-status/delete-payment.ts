...
export const handler = async (event: APIGatewayProxyEventV2WithJWTAuthorizer) => {
  try {
    ...
    const { data: transactionsUsingPaymentStatus, error: transactionCheckError } = await supabase
        .from("transactions")
        .select("id")
    ...
    if (transactionsUsingPaymentStatus && transactionsUsingPaymentStatus.length > 0) {
        return {
            statusCode: 409,
            body: JSON.stringify({
